...
let of_lang_or_family lang = if is_part5 lang then Some lang else None
...
let of_int_exn i =
  ...
  if is_part5 lang then lang else failwith "Iso639.Langfamily.of_int_exn"

let of_string = of_part5_string

let to_string lang =
  (match  to_part5_string lang with
   | Some s -> s
   | None -> assert false)
